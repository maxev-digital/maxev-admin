...
import {
  ...
  ChevronRight, User, Building2, Search, X, Check, AlertCircle,
  Mail, MailOpen, Loader2, ExternalLink,
} from 'lucide-react';
...
interface EmailAccountSummary {
  id: string; label: string; email: string; lastSyncAt: string | null;
}

export default function InboxPage() {
  const searchParams = useSearchParams();
  const [accounts,    setAccounts]    = useState<EmailAccountSummary[]>([]);
  const [activeAcct,  setActiveAcct]  = useState<string>('');  // '' = all
  ...
  const loadMessages = useCallback(async (f: string, q: string, cId?: string, aId?: string) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    loadMessages(folder, search, clientFilter || undefined, activeAcct || undefined);
  }, [folder, loadMessages, clientFilter, activeAcct]); // eslint-disable-line react-hooks/exhaustive-deps

  // Auto-open messageId from URL param (e.g. from client detail page)
  useEffect(() => {
  ...
  // Load accounts + IMAP config on mount
  useEffect(() => {
  ...
  async function triggerSync() {
    ...
    try {
      ...
      if (data.demo || (!data.ok && data.message)) {
        ...
        await loadMessages(folder, search, clientFilter || undefined, activeAcct || undefined);
      }
  ...
  function handleSearch(q: string) {
    setSearch(q);
    if (q.length > 1 || q === '') loadMessages(folder, q);
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--gray)', fontSize: '0.82rem' }}>
              ...
              {search ? 'No results' : folder === 'INBOX' ? 'No messages yet' : `No ${folder.toLowerCase()} messages`}
            ...
            <div
              ...
              onMouseEnter={(e) => { if (selected?.id !== msg.id) (e.currentTarget as HTMLDivElement).style.background = 'rgba(255,255,255,0.03)'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = selected?.id === msg.id ? 'rgba(59,125,217,0.1)' : msg.isRead ? 'transparent' : 'rgba(59,125,217,0.04)'; }}
            >
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 24, overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
              {[
                ...
                { label: 'Subject', key: 'subject', placeholder: 'Subject line' },
              ].map(({ label, key, placeholder }) => (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--gray)', width: 54, flexShrink: 0 }}>{label}</span>
                  <input
                    className="input"
                    placeholder={placeholder}
                    value={compose[key as keyof ComposeState] as string}
                    onChange={(e) => setCompose((c) => ({ ...c, [key]: e.target.value }))}
              ...
              <textarea
                ...
                style={{
                  flex: 1, minHeight: 260, background: 'var(--card2)', border: '1px solid var(--border)',
            ...
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button onClick={() => toggleStar(selected, { stopPropagation: () => {} } as React.MouseEvent)} className="btn btn-ghost btn-sm" style={{ color: selected.isStarred ? '#f59e0b' : undefined }}>
                    <Star size={12} fill={selected.isStarred ? '#f59e0b' : 'none'} />
                  </button>
                </div>
              </div>

              {/* Thread — show all messages */}
              {thread.map((msg, i) => (
...
// Basic HTML sanitizer — strip scripts/event handlers, keep structure
